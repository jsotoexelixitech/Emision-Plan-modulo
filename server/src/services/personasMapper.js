...
function buildEmissionPersonRequest(state, cotizacion, overrides = {}) {
  ...
  const payload = {
    ...
    // ── Titular (1er asegurado; cae al tomador para datos de contacto/domicilio) ─
    tipo_cedula_titular: normalizeTipoCedula(titular.tipoDoc || tomador.tipoDoc),
    rif_titular: digitsToNumber(titular.identificacion || tomador.identificacion),
    ...
    estado_civil_titular: normalizeEstadoCivil(titular.estadoCivil || tomador.estadoCivil),
    fnac_titular: normalizeDate(titular.fechaNac || tomador.fechaNac),
    estado_titular: estado_tomador,
